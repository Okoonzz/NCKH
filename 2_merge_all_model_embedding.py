...
print("2: all-MiniLM-L6-v2 (SentenceTransformer)")
choice = input("Enter 1 or 2 [default=1]: ").strip()
if choice == '2':
    EMBED_MODEL = 'all-MiniLM-L6-v2'
    OUTPUT_DIR = 'pyg_data_MiniLML6v2'
else:
...
def graphml_to_pyg_data(path):
    try:
        ...
        return Data(x=x, edge_index=edge_idx, y=y, graph_id=G.graph.get('graph_id', os.path.basename(path)))
...
def process_and_save(path, out_dir):
    ...
    if data is None:
        return
    data.x = data.x.cpu(); data.edge_index = data.edge_index.cpu(); data.y = data.y.cpu()
